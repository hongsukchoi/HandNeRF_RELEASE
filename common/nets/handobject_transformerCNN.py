...
class HandObjectCorrelationEncoderTransformerCNN(nn.Module):
    def __init__(self):
        super(HandObjectCorrelationEncoderTransformerCNN, self).__init__()

        # Method of multi-input view feature aggregation
        self.multiview_feat_aggregation = 'average'

        # For semantic latent encoding of (hand vertex | object point) features
        latent_path = '../data/latent.pt'  # assume the current directory is './main'
        latents = torch.load(latent_path)  # just gaussian random tensor
        self.register_buffer(
            "hand_latent", latents['hand_latent'], persistent=True)  # (778,10)
        self.register_buffer(
            "object_latent", latents['object_latent'], persistent=True)  # (1,10)
        self.latent_dim = 10

        if 'latenthandmesh' in cfg.nerf_mode:
            latents_64 = torch.load('../data/latent_64.pt')
            self.register_buffer(
                "hand_latent64", latents_64['hand_latent'], persistent=True)  # (778,64)
            self.register_buffer(
                "object_latent64", latents_64['object_latent'], persistent=True)  # (1,64)

        self.mlp_q1 = nn.Linear(64+3+self.latent_dim, 64)
        self.mlp_k1 = nn.Linear(64, 64)
        self.mlp_v1 = nn.Linear(64, 64)
        
        self.mlp_q2 = nn.Linear(64+3+self.latent_dim, 64)
        self.mlp_k2 = nn.Linear(64, 64)
        self.mlp_v2 = nn.Linear(64, 64)

        mlp_list = [self.mlp_q1, self.mlp_k1, self.mlp_v1, self.mlp_q2, self.mlp_k2, self.mlp_v2]
        for mlp in mlp_list:
            _xavier_init(mlp)
    ...
    def forward(self, world_3d_data, input_view_parameters, feature_map, mode):
        ...
        # (cfg.num_input_views, 3, 3)
        input_view_R = input_view_parameters['input_view_R'][0]
        # (cfg.num_input_views, 3, 1)
        input_view_t = input_view_parameters['input_view_t'][0]

        # (1, 778, 3), mano hand mesh
        world_hand_mesh = world_3d_data['world_hand_mesh']
        # (1, 21, 3), mano hand joints
        world_hand_joints = world_3d_data['world_hand_joints']
        # world2manojoints = world_3d_data['world2manojoints']  # (1, 15, 4, 4)

        # (1, 778, 3), discretized world_mesh.
        world_hand_mesh_voxel_coord = world_3d_data['world_hand_mesh_voxel_coord']
        # (1, 3), boundary of hand+object
        world_mesh_voxel_out_sh = world_3d_data['world_mesh_voxel_out_sh']
        ...
        if 'latenthandmesh' in cfg.nerf_mode:
            ...
            input_camera_hand_vertices = torch.bmm(input_view_R, world_hand_mesh.transpose(
                1, 2).expand(input_view_R.shape[0], -1, -1)).transpose(1, 2) + input_view_t.view(-1, 1, 3)
            # input_camera_hand_vertices: (cfg.num_input_views, 778, 3)

            input_img_xyz = torch.bmm(
                input_camera_hand_vertices, input_view_K.transpose(1, 2))
            input_img_hand_vertices = input_img_xyz[:,
                                                    :, :2] / input_img_xyz[:, :, 2:]
            # input_img_hand_vertices: (cfg.num_input_views,778, 2)

            # normalize to [-1,1]
            normalized_input_img_hand_vertices = input_img_hand_vertices / \
                torch.Tensor([cfg.input_img_shape[1]-1, cfg.input_img_shape[0]-1]
                             )[None, None, :].to(input_img_hand_vertices) * 2. - 1

            input_img_hand_mesh_feat = F.grid_sample(
                feature_map, normalized_input_img_hand_vertices[:, :, None, :], padding_mode=cfg.feature_map_padding_mode, align_corners=True)[:, :, :, 0]
            # input_img_hand_mesh_feat: (cfg.num_input_views, C, 778)

        input_img_hand_mesh_feat = input_img_hand_mesh_feat.permute(
            0, 2, 1)  # (cfg.num_input_views, 778, C)
        ...
        world_object_points = world_3d_data[
            'world_object_points']  # (1, -1 , 3), 3D object points inside the 2D masks
        # (1, -1, 3), discretized voxel coord. dhw
        world_object_points_voxel_coord = world_3d_data['world_object_points_voxel_coord']

        if world_object_points.sum() != 0:
            if 'latenthandmesh' in cfg.nerf_mode:
                input_img_object_points_feat = self.object_latent64.transpose(
                    0, 1)[None].repeat(1, 1, world_object_points.shape[1])
            else:
                input_camera_object_points = torch.bmm(input_view_R, world_object_points.transpose(
                    1, 2).expand(input_view_R.shape[0], -1, -1)).transpose(1, 2) + input_view_t.view(-1, 1, 3)
                # input_camera_object_points: (cfg.num_input_views, -1, 3)

                input_img_xyz = torch.bmm(
                    input_camera_object_points, input_view_K.transpose(1, 2))
                input_img_object_points = input_img_xyz[:,
                                                        :, :2] / input_img_xyz[:, :, 2:]
                # input_img_object_points: (cfg.num_input_views, -1, 2)

                # normalize to [-1,1]
                normalized_input_img_object_points = input_img_object_points / \
                    torch.Tensor([cfg.input_img_shape[1]-1, cfg.input_img_shape[0]-1]
                                 )[None, None, :].to(input_img_object_points) * 2. - 1

                input_img_object_points_feat = F.grid_sample(
                    feature_map, normalized_input_img_object_points[:, :, None, :], padding_mode=cfg.feature_map_padding_mode, align_corners=True)[:, :, :, 0]
                # input_img_object_points_feat: (cfg.num_input_views, C, -1)

            input_img_object_points_feat = input_img_object_points_feat.permute(
                0, 2, 1)  # (cfg.num_input_views, -1, C)
            if self.multiview_feat_aggregation == 'average':
                object_points_feat = input_img_object_points_feat.mean(
                    dim=0, keepdim=True)
            else:
                raise ValueError(
                    "No other mesh feature aggregation type defined!")

        """ Positional and Semantic encoding """
        batch_size = 1
        # positional encoding
        pe_hand_mesh_feat = self.positional_encoding(
            world_hand_mesh, hand_mesh_feat, world_hand_joints)
        # pe_hand_mesh_feat: (1, 778, C+3)
        # semantic encoding; assume batch 1; [None, :, :]
        pe_se_hand_mesh_feat = torch.cat(
            [pe_hand_mesh_feat, self.hand_latent[None, :, :]], dim=-1)
        # pe_se_hand_mesh_feat: (1, N, C+3+10)

        # reshape
        pe_se_hand_mesh_feat = pe_se_hand_mesh_feat.view(
            batch_size * pe_se_hand_mesh_feat.shape[1], -1)  # (batch_size * 778, C+3+10)

        if world_object_points.sum() != 0:
            # positional encoding
            pe_object_points_feat = self.positional_encoding(
                world_object_points, object_points_feat, world_hand_joints)
            # pe_object_points_feat: (1, N, C+3)
            # semantic encoding; assume batch 1; [None, :, :]
            pe_se_object_points_feat = torch.cat([pe_object_points_feat, self.object_latent[None, :, :].repeat(
                1, pe_object_points_feat.shape[1], 1)], dim=-1)
            # pe_se_object_points_feat: (1, N, C+3+10)

            # reshape
            pe_se_object_points_feat = pe_se_object_points_feat.view(
                batch_size * pe_se_object_points_feat.shape[1], -1)  # (batch_size * N, C+3+10)

        else:  # no object points
            # concat hand and object features
            pe_3d_feat = pe_hand_mesh_feat  # (batch_size * 778, C+3+10)
            world_3d_voxel_coord = world_hand_mesh_voxel_coord  # (1, 778, 3)

        """ QKV attention for hand features """
        q1 = self.mlp_q1(pe_se_hand_mesh_feat) # (778, C+3+10) -> (778, C)
        k1 = self.mlp_k1(object_points_feat[0])  # (1, N, C) -> (N, C)
        v1 = self.mlp_v1(object_points_feat[0])  # (N, C) -> (N, C)

        # get attention
        att1 = q1 @ k1.T # (778, N)
        att1 = F.softmax(att1, dim=-1)  # (778, N)
        x1 = att1 @ v1  # (778, C)

        # residual 
        q1 = q1 + x1  # hand mesh feat
        
        """ QKV attention for hand features """
        q2 = self.mlp_q2(pe_se_object_points_feat) # (N, C+3+10) -> (N, C)
        k2 = self.mlp_k2(hand_mesh_feat[0])  # (778, C) -> (778, C)
        v2 = self.mlp_v2(hand_mesh_feat[0])  # (778, C) -> (778, C)

        # get attention
        att2 = q2 @ k2.T # (N, 778)
        att2 = F.softmax(att2, dim=-1)  # (N, 778)
        x2 = att2 @ v2  # (N, C)

        # residual 
        q2 = q2 + x2  # obj point feat
           
        # Concat object and hand features
        pe_3d_feat = torch.cat([q1, q2], dim=0)  # (batch_size * (778 + N), C)
        world_3d_voxel_coord = torch.cat([world_hand_mesh_voxel_coord, world_object_points_voxel_coord, ], dim=1)  # (1,  778 + N, 3)
            
        # if you need faster running, skip this. This is for deterministic evaluation.
        if mode != 'train':
            val, inv, cnt, idx = deterministic_unique(
                world_3d_voxel_coord, dim=1)
            pe_3d_feat = pe_3d_feat[idx]
            world_3d_voxel_coord = world_3d_voxel_coord[:, idx]

        """ Make multi-scale volumes, used for tri-linear interpolation. No CNN """
        # Downsample feature volume
        feature_volume_list = self.encode_sparse_voxels(
            pe_3d_feat, world_3d_voxel_coord, world_mesh_voxel_out_sh, batch_size)
    ...
    def positional_encoding(self, points, points_feat, world_hand_joints):
        ...
        pe_points_feat = torch.cat(
            [points_feat, norm_points], dim=-1)  # (1, N, C+3)
    ...
    def encode_sparse_voxels(self, mesh_feat, coord, out_sh, batch_size):
        ...
        xyzc = spconv.SparseConvTensor(
            mesh_feat, sp_coord, sp_out_sh, batch_size)
